...
def build_album_list():
    ...
    photo_keys = ['uuid', 'filename', 'original_filename', 'date', 'description', 'title', 'keywords',
                  'labels', 'albums', 'path', 'path_edited']
    albums = {}

    for a in photos_db.album_info:
        album = dict([(k, getattr(a, k)) for k in album_keys])
        album['photos'] = [dict([(k, getattr(p, k)) for k in photo_keys]) for p in a.photos]
...
def sync_photo(photo):
    ...
    r = requests.put(f"{base_url}/api/photos/{p['uuid']}/", data=json.dumps(p, cls=DateTimeEncoder),
                     headers={'Content-Type': 'application/json', "Authorization": f"Token {token}"})

    if r.status_code == 404:
        r = requests.post(f"{base_url}/api/photos/", data=json.dumps(p, cls=DateTimeEncoder),
                          headers={'Content-Type': 'application/json', "Authorization": f"Token {token}"})
...
def upload_albums():
    for album_info in photos_db.album_info:
        ...
        if r.status_code == 404:
            r = requests.post(f"{base_url}/api/albums/", data=json.dumps(album, cls=DateTimeEncoder),
                              headers={'Content-Type': 'application/json', "Authorization": f"Token {token}"})
