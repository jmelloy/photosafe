    ...
    def test_import_with_meta_json(self, runner, test_user, test_library):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(import_photos, [
                ...
                '--library-id', str(test_library.id),
            ...
            try:
                ...
                if photo.fields:
                    fields_data = json.loads(photo.fields) if isinstance(photo.fields, str) else photo.fields
                    assert 'photographer' in fields_data or 'fields' in fields_data
            finally:
                db.close()
    
    def test_import_with_exif_extraction(self, runner, test_user, test_library, temp_image_with_exif):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if photo.exif:
                    exif_data = json.loads(photo.exif) if isinstance(photo.exif, str) else photo.exif
                    assert isinstance(exif_data, dict)
