...
@click.option(
    "--expires-in-days", type=int, help="Number of days until token expires (optional)"
)
...
def list_tokens(username: str):
    ...
    try:
        ...
        for token in tokens:
            ...
            if (
                token.expires_at
                and datetime.now(token.expires_at.tzinfo or None) > token.expires_at
            ):
                expires = f"{expires} (EXPIRED)"

            click.echo(
                f"{token.id:<6} {token.name:<30} {created:<20} {last_used:<20} {expires:<20}"
            )
        click.echo()
    finally:
        db.close()
...
def revoke_token(username: str, token_id: int):
    ...
    try:
        ...
        if success:
            ...
            click.echo(
                f"Error: Token {token_id} not found or not owned by user", err=True
            )
    except Exception as e:
        db.rollback()
