...
def demo_photo_management(token):
    ...
    response = requests.post(f"{BASE_URL}/api/photos/", json=photo_data, headers=headers)
    if response.status_code == 201:
    ...
    update_data = {
        "title": "Updated Demo Photo"
    }
    response = requests.patch(f"{BASE_URL}/api/photos/{photo_uuid}/", json=update_data, headers=headers)
    if response.status_code == 200:
...
def demo_authorization():
    ...
    # Login both users
    token1 = requests.post(f"{BASE_URL}/api/auth/login", 
                          data={"username": "user1", "password": "password123"}).json()["access_token"]
    token2 = requests.post(f"{BASE_URL}/api/auth/login", 
                          data={"username": "user2", "password": "password123"}).json()["access_token"]
    ...
    response = requests.post(f"{BASE_URL}/api/photos/", 
                            json=photo_data, 
    ...
    response = requests.get(f"{BASE_URL}/api/photos/{photo_uuid}/", 
    ...
    response = requests.delete(f"{BASE_URL}/api/photos/{photo_uuid}/", 
                              headers={"Authorization": f"Bearer {token2}"})
    if response.status_code == 403:
