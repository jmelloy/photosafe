...
class TestMacOS404Handling:
    ...
    def test_macos_sync_404_creates_photo_with_original(self, runner, macos_sample_data):
    ...
    def test_macos_sync_404_skips_trashed_photo(self, runner, macos_sample_data):
        """Test that 404 response skips photo if it's in trash"""
        mock_osxphotos = MagicMock()
        with patch.dict("sys.modules", {"osxphotos": mock_osxphotos}):
            # Create mock PhotosDB
            mock_photos_db = MagicMock()
            ...
            # Use a photo that's in trash
            photo_data = macos_sample_data[0].copy()
            photo_data["path"] = "/originals/test/photo.jpg"
            photo_data["intrash"] = True  # In trash
            ...
            with patch("cli.sync_commands.boto3"):
                ...
                with patch("cli.sync_tools.PhotoSafeAuth") as mock_auth:
                    ...
                    assert result.exit_code == 0
                    # Should skip the photo
                    assert "Skipping missing photo" in result.output
            ...
            with patch("cli.sync_commands.boto3"):
                ...
                with patch("cli.sync_tools.PhotoSafeAuth") as mock_auth:
                    ...
                    with patch("cli.sync_commands.os.path.exists", return_value=False):
                        ...
                        assert "file missing locally" in result.output
