...
class TestMacOSSyncWithFixtures:
        ...
        with patch("cli.sync_commands.osxphotos") as mock_osxphotos:
            ...
            with patch("cli.sync_commands.boto3") as mock_boto3:
                ...
                with patch("cli.sync_commands.PhotoSafeAuth") as mock_auth:
                    ...
                    assert "photos processed" in result.output or "to process" in result.output
        ...
        with patch("cli.sync_commands.osxphotos") as mock_osxphotos:
            ...
            with patch("cli.sync_commands.boto3") as mock_boto3:
                ...
                with patch("cli.sync_commands.PhotoSafeAuth") as mock_auth:
                    ...
                    mock_auth_instance.get.return_value.json.return_value = server_blocks
                    mock_auth_instance.get.return_value.status_code = 200
                    mock_auth_instance.get.return_value.raise_for_status = MagicMock()
    ...
    def test_macos_sync_date_comparison(self, runner, macos_sample_data):
        ...
        with patch("cli.sync_commands.osxphotos") as mock_osxphotos:
            ...
            mock_photo.date_modified = parser.parse(photo_data["date_modified"]) if photo_data.get("date_modified") else photo_date
            mock_photo._info = {
                "cloudAssetGUID": photo_data["uuid"],
                "masterFingerprint": photo_data.get("masterFingerprint")
            }
            ...
            with patch("cli.sync_commands.boto3") as mock_boto3:
                ...
                with patch("cli.sync_commands.PhotoSafeAuth") as mock_auth:
                    ...
                    older_date = datetime(dt.year, dt.month, dt.day, 0, 0, 0, tzinfo=timezone.utc)
